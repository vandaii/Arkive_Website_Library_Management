<x-layouts.admin-dashboard>
    <x-slot:title>Pengajuan Pinjaman</x-slot:title>
    <div class="bg-white px-10 py-8 rounded-lg">
        <div class="flex justify-between items-center mb-8">
            <div>
                <h1 class="text-2xl font-medium">Pengajuan Pinjaman</h1>
                <p class="text-sm text-black/50 mt-1">Daftar pengajuan peminjaman buku yang menunggu persetujuan</p>
            </div>
            <a href="{{ route('kelola-pinjam.index') }}"
                class="group flex items-center gap-1.5 px-4 py-2.5 text-black/50 rounded-lg hover:text-black transition-all duration-200 text-sm">
                <i class="size-4 rotate-180 text-black/50 group-hover:text-black" data-lucide="arrow-right"></i>Kembali
            </a>
        </div>
    ...
    <x-detail-modal id="pengajuanPinjamModal" title="Detail Pengajuan Pinjaman">
        <div class="flex gap-5 mb-5">
            <img id="pengajuanPinjamCover" class="w-24 h-36 object-cover rounded-lg shadow-md shrink-0" src=""
                alt="Cover">
            <div class="flex-1">
        ...
        <div class="border border-black/10 rounded-xl p-4">
            ...
            <div class="grid grid-cols-2 gap-x-6">
                <div class="modal-info-row">
                    ...
                    <span class="modal-info-value" id="pengajuanPinjamInfoUser">-</span>
                </div>
                <div class="modal-info-row">
                    <span class="modal-info-label"><i class="size-3.5" data-lucide="mail"></i>Email</span>
                    <span class="modal-info-value" id="pengajuanPinjamInfoEmail">-</span>
                </div>
                <div class="modal-info-row">
                    <span class="modal-info-label"><i class="size-3.5" data-lucide="book-open"></i>Judul Buku</span>
                    <span class="modal-info-value" id="pengajuanPinjamInfoTitle">-</span>
                </div>
                <div class="modal-info-row">
                    <span class="modal-info-label"><i class="size-3.5" data-lucide="phone"></i>No. Handphone</span>
                    <span class="modal-info-value" id="pengajuanPinjamInfoPhoneNumber">-</span>
                </div>
                <div class="modal-info-row">
                    <span class="modal-info-label"><i class="size-3.5" data-lucide="calendar"></i>Tgl Pinjam</span>
                    <span class="modal-info-value" id="pengajuanPinjamInfoDate">-</span>
                </div>
                <div class="modal-info-row">
                    <span class="modal-info-label"><i class="size-3.5" data-lucide="calendar-check"></i>Est.
                        Kembali</span>
                    <span class="modal-info-value" id="pengajuanPinjamInfoReturn">-</span>
                </div>
            </div>
        </div>
    </x-detail-modal>
    ...
    <script>
        ...
        function openPengajuanDetail(id) {
            ...
            document.getElementById('pengajuanPinjamCover').src = storageBase + (p.buku ? p.buku.cover_buku : '');
            document.getElementById('pengajuanPinjamTitle').textContent = p.buku ? p.buku.judul : '-';
            document.getElementById('pengajuanPinjamAuthor').textContent = p.buku ? p.buku.penulis : '-';
            document.getElementById('pengajuanPinjamUserName').textContent = 'Peminjam: ' + (p.user ? p.user.nama_lengkap :
                '-');
            document.getElementById('pengajuanPinjamInfoUser').textContent = p.user ? p.user.nama_lengkap : '-';
            document.getElementById('pengajuanPinjamInfoEmail').textContent = p.user ? p.user.email : '-';
            document.getElementById('pengajuanPinjamInfoTitle').textContent = p.buku ? p.buku.judul : '-';
            document.getElementById('pengajuanPinjamInfoPhoneNumber').textContent = p.user ? p.user.phone_number : '-';
            document.getElementById('pengajuanPinjamInfoDate').textContent = p.tanggal_peminjaman || '-';
            document.getElementById('pengajuanPinjamInfoReturn').textContent = p.estimasi_tanggal_pengembalian || p
                .tanggal_pengembalian || '-';

            openModal('pengajuanPinjamModal');
        }
