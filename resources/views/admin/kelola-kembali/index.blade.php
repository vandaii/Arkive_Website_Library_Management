<x-layouts.admin-dashboard>
    ...
    <div class="ml-45 bg-white px-10 py-8 rounded-lg">
        ...
        <div class="flex justify-between items-center mb-5">
            ...
            <div class="relative">
                <a class="px-5 py-3 bg-indigo-500 text-white rounded-lg" href="{{ route('reports.pengembalian') }}">Lihat
                    Laporan</a>
        ...
        <table class="table-fixed w-full">
            <thead>
                <tr class="border-b-2 border-gray-500/40 text-left">
                    ...
                    <th class="w-2/12 py-1.5">Tanggal Kembali</th>
            ...
            <tbody>
                @forelse ($pengembalians as $pengembalian)
                    <tr class="border-b-2 border-gray-500/40">
                        <td class="py-1.5 line-clamp-2">{{ $pengembalian->buku->judul }}</td>
                        <td>{{ $pengembalian->user->nama_lengkap }}</td>
                        <td>{{ $pengembalian->tanggal_peminjaman }}</td>
                        <td>{{ $pengembalian->tanggal_pengembalian }}</td>
                        <td>{{ $pengembalian->stok }}</td>
