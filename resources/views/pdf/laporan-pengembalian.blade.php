...
<head>
    <meta charset="UTF-8">
    <title>Laporan Data Pengembalian - Arkive</title>
    <style>
    ...
    <div class="container">
        <div class="header">
            ...
            <div class="header-right">
                <div class="doc-type">Laporan Data Pengembalian</div>
                <div class="doc-date">Dicetak: {{ $tanggal }}</div>
                <div class="doc-date">Total: {{ $pengembalians->count() }} data</div>
            </div>
        </div>
                ...
                @foreach ($pengembalians as $i => $p)
                    <tr>
                        ...
                        <td class="text-center">
                            <span class="badge {{ $p->status_peminjaman == 'Terlambat' ? 'badge-terlambat' : 'badge-dikembalikan' }}">
                                {{ $p->status_peminjaman }}
                            </span>
                        </td>
                    </tr>
        ...
        <div class="summary">
            <p>Tepat Waktu: <strong>{{ $pengembalians->where('status_peminjaman', 'Dikembalikan')->count() }}</strong> |
               Terlambat: <strong>{{ $pengembalians->where('status_peminjaman', 'Terlambat')->count() }}</strong>
            </p>
        </div>
