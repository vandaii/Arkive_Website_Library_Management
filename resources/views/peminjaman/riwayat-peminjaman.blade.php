    ...
    <div class="container mx-auto px-6 py-8">
        <div class="flex flex-col gap-y-8">
            <div class="p-5 rounded-2xl bg-white">
                ...
                <div>
                    <form class="flex gap-3 items-center" action="{{ route('peminjaman.riwayat-peminjaman') }}"
                        method="GET">
                        <div class="flex-1">
                            <div class="relative">
                                ...
                                <i class="absolute top-2.5 left-4 size-4 my-auto stroke-black/40"
                        ...
                        <div class="relative">
                            <select
                                ...
                                <option value="All"
                                    {{ request('status_peminjaman') == 'All' || !request('status_peminjaman') ? 'selected' : '' }}>
                                    Semua Status</option>
                                <option value="Pending"
                                    {{ request('status_peminjaman') == 'Pending' ? 'selected' : '' }}>Pending</option>
                                <option value="Dipinjam"
                                    {{ request('status_peminjaman') == 'Dipinjam' ? 'selected' : '' }}>Dipinjam</option>
                                <option value="Pending Dikembalikan"
                                    {{ request('status_peminjaman') == 'Pending Dikembalikan' ? 'selected' : '' }}>
                                    Pending Dikembalikan</option>
                                <option value="Dikembalikan"
                                    {{ request('status_peminjaman') == 'Dikembalikan' ? 'selected' : '' }}>Dikembalikan
                                </option>
                                <option value="Terlambat"
                                    {{ request('status_peminjaman') == 'Terlambat' ? 'selected' : '' }}>Terlambat
                                </option>
                            </select>
                        ...
                        <button type="submit"
                            class="bg-(--third-color) text-white px-4 py-2 rounded-full text-sm font-medium hover:bg-(--second-color) transition-all duration-200 cursor-pointer">
            ...
            <div class="flex gap-5 flex-col w-full">
                @forelse ($historys as $history)
                    <div
                        class="group relative flex items-center gap-5 p-5 rounded-2xl transition-all hover:shadow-lg bg-white">
                        <a class="flex items-center gap-5 flex-1 min-w-0"
                            ...
                            <div class="flex-1 min-w-0">
                                <div class="flex items-start justify-between gap-3 mb-2">
                                    ...
                                    @endphp
                                    <span
                                        class="px-3 py-1 rounded-full shrink-0 text-xs font-semibold {{ $statusColor }}">
                                        {{ $history->status_peminjaman }}
                                    </span>
                                </div>
                                <div class="flex flex-wrap gap-x-6 gap-y-1 text-sm">
                                    <h2 class="text-black/50 text-xs">Tanggal Peminjaman: <span
                                            class="text-black font-medium text-sm">{{ $history->tanggal_peminjaman }}</span>
                                    </h2>
                                    @if (in_array($history->status_peminjaman, ['Dikembalikan', 'Terlambat']))
                                        <h2 class="text-black/50 text-xs">Tanggal Pengembalian: <span
                                                class="text-black font-medium text-sm">{{ $history->tanggal_pengembalian }}</span>
                                        </h2>
                                    @else
                        ...
                        @if (in_array($history->status_peminjaman, ['Dikembalikan', 'Terlambat']))
                            ...
                            @else
                                <span
                                    class="flex absolute right-4 bottom-2 items-center gap-1.5 px-3 py-2 rounded-full text-xs font-medium text-green-600 bg-green-50 shrink-0">
        ...
        <div class="flex items-center justify-center min-h-screen p-4">
            <div class="relative bg-white rounded-2xl shadow-2xl w-full max-w-lg p-8 z-10 animate-modal-in">
                <button onclick="closeReviewModal()"
                    class="absolute top-4 right-4 text-black/30 hover:text-black transition-colors cursor-pointer">
                ...
                <form action="{{ route('ulasan.store') }}" method="POST">
                        ...
                        <div class="flex items-center gap-1" id="modalStarContainer">
                            @for ($i = 1; $i <= 5; $i++)
                                <i data-lucide="star" data-value="{{ $i }}"
                                    class="modal-star cursor-pointer w-8 h-8 fill-(--third-color) text-(--third-color) stroke-1 hover:scale-110 transition-transform"></i>
                            @endfor
                        </div>
                    </div>
                    ...
                    <div class="flex gap-3">
                        ...
                        <button type="submit"
                            class="flex-1 py-2.5 rounded-full bg-(--third-color) text-white font-medium text-sm hover:bg-(--second-color) transition-all duration-200 cursor-pointer">Kirim
                            Ulasan</button>
                    </div>
                </form>
            </div>
